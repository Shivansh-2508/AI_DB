# -------------------- AI Table Cache (for chart generation) --------------------
import os
...
def cache_ai_table(session_id, columns, rows):
    """
    Cache the full table data generated by the AI for a given session.
    """
    if columns and rows:
...
def get_cached_ai_table(session_id):
    """
    Fetch the cached table data for a session (for chart generation).
    """
...
def detect_chartable(columns, rows):
    """
    Decide if the query results are suitable for charting.
    Returns (chartable: bool, chart_type: str|None).
    """
    if not columns or not rows:
        return False, None

    # Must have at least 2 columns to chart
    if len(columns) < 2:
        return False, None
    ...
    try:
        ...
        numeric = all(
            isinstance(v, (int, float)) or (isinstance(v, str)
